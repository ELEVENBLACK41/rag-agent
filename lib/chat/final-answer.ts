 ...
 */
/**
 * 修改时间：2026-09-18 | 文件说明：依据本轮本地与网页证据构造最终回答说明 | edit by：Sliye
 */

import { MAX_SOURCE_CHARACTERS } from "@/lib/agent/run-state";
import { isDisplayableImageCitation } from "@/lib/chat/citations";
...
export const finalAnswerSchema = z.object({
  ...
  imageCitationIds: z
    ...
    .max(4)
    .describe("用户明确要求查找或展示图片时，填写最多 4 个 canDisplayImage=true 的来源编号；否则为空数组"),
});
...
export function buildFinalInstruction(context: FinalAnswerContext) {
  ...
  return [
    ...
    "当前已经进入资料回答阶段。知识性事实只能依据以下已读取正文证据、文件清单元数据或实际网页搜索摘要；没有相应证据时如实说明缺口，不用模型一般知识替代知识库结果。",
    ...
    "不得静默纠正用户原话。若来源支持其字面意思，按字面回答；若来源不支持且上下文只有一个明显意图，可以采用该解释，但在 answer 开头用一句话说明‘我按……理解’；若存在多个会改变答案的合理解释，只说明已确认的信息并提出一个具体澄清问题，不擅自选择。",
    ...
    JSON.stringify({
      ...
      sources: sources.map((source) => ({
        ...
        sourceKind: isDisplayableImageCitation(source) ? "visual" : "text",
        canDisplayImage: isDisplayableImageCitation(source),
