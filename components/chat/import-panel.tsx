...
export function ImportPanel({ batch, error, files, isLoading, isUploading, onRemoveFile, onUpload }: ImportPanelProps) {
  ...
  return (
    ...
    <Card className="min-w-0 rounded-2xl ring-border [--card-spacing:--spacing(5)]">
      <CardHeader>
        <CardTitle className="flex items-center justify-between gap-3">现有知识库文件<Badge variant="secondary" className="rounded-md tabular-nums">{isLoading ? "读取中" : `${files.length} 个文件`}</Badge></CardTitle>
        <CardDescription className="leading-relaxed">列表来自当前已发布快照；同一路径再次导入会生成新版本，并替换后续 AI 检索所用版本。</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading && <p role="status" className="rounded-xl bg-muted/40 p-5 text-sm text-muted-foreground">正在读取知识库文件…</p>}
        {!isLoading && files.length === 0 && <div className="rounded-xl bg-muted/30 px-5 py-12 text-center"><FileTextIcon className="mx-auto mb-4 size-7 text-muted-foreground" /><p className="font-medium">知识库还是空的</p><p className="mt-2 text-sm leading-relaxed text-muted-foreground">上传第一批资料，完成后会自动出现在这里。</p></div>}
